...
class InvokePlugin implements PluginInterface
{
    ...
    public function assembly(Rocket $rocket, Closure $next): Rocket
    {
        ...
        Logger::debug('[Wechat][V3][Pay][App][InvokePlugin] 插件开始装载', ['rocket' => $rocket]);

        $destination = $rocket->getDestination();
        $prepayId = $destination?->get('prepay_id');

        if (is_null($prepayId)) {
            Logger::error('[Wechat][V3][Pay][App][InvokePlugin] 预下单失败：响应缺少 `prepay_id` 参数，请自行检查参数是否符合微信要求', $destination?->all() ?? null);
        ...
        Logger::info('[Wechat][V3][Pay][App][InvokePlugin] 插件装载完毕', ['rocket' => $rocket]);
