        ...
        <el-card class="box-card !border-none" shadow="never">
            <el-form :model="formData" label-width="90px" ref="formRef" :rules="formRules" class="page-form">
                ...
                <el-form-item :label="t('selectedIcon')" prop="selected_icon">
                    <el-input v-model="formData.selected_icon" clearable :placeholder="t('selectedIconPlaceholder')" class="input-width" />
                </el-form-item>
...
import { getSportNavConfigInfo,addSportNavConfig,editSportNavConfig } from '@/addon/sport/api/sport_nav_config';
...
const setFormData = async (id:number = 0) => {
    ...
    const data = await (await getSportNavConfigInfo(id)).data
...
,
    selected_icon: [
        { required: true, message: t('selectedIconPlaceholder'), trigger: 'blur' },
...
const onSave = async (formEl: FormInstance | undefined) => {
    ...
    await formEl.validate(async (valid) => {
       if (valid) {
           ...
           const save = id ? editSportNavConfig : addSportNavConfig
           save(data).then(res => {
               loading.value = false
               history.back()
           }).catch(err => {
