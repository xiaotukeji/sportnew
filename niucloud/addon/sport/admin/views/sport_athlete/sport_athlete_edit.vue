        ...
        <el-card class="box-card !border-none" shadow="never">
            <el-form :model="formData" label-width="90px" ref="formRef" :rules="formRules" class="page-form">
                ...
                <el-form-item :label="t('photo')" >
                    <el-input v-model="formData.photo" clearable :placeholder="t('photoPlaceholder')" class="input-width" />
                </el-form-item>
...
import { getSportAthleteInfo,addSportAthlete,editSportAthlete } from '@/addon/sport/api/sport_athlete';
...
const setFormData = async (id:number = 0) => {
    ...
    const data = await (await getSportAthleteInfo(id)).data
...
const onSave = async (formEl: FormInstance | undefined) => {
    ...
    await formEl.validate(async (valid) => {
       if (valid) {
           ...
           const save = id ? editSportAthlete : addSportAthlete
           save(data).then(res => {
               loading.value = false
               history.back()
           }).catch(err => {
