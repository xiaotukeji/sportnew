...
class CorePrinterService extends BaseCoreService
{
    ...
    {
        return ( new ConfigService() )->setConfig('PRINTER_YLY_TOKEN', $data);
    }
    ...
    {
        $info = ( new ConfigService() )->getConfig('PRINTER_YLY_TOKEN');
        if (empty($info)) {
            $info = [];
            $info[ 'value' ] = [
                'access_token' => '',
     ...
     * @return mixed
     */
    public function getYlyToken($yly_config, $refresh = false)
    {
        // token 配置
        $config = $this->getYlyTokenConfig();
     ...
     * @return mixed
     */
    public function addPrinterYly($params)
    {
     ...
     * @return mixed
     */
    public function deletePrinterYly($params)
    {
        $yly_config = new YlyConfig($params[ 'open_id' ], $params[ 'apikey' ]);
        $access_token = $this->getYlyToken($yly_config);
    ...
    public function refreshToken($printer_id)
    {
        ...
        $info = ( new SysPrinter() )->field($field)->where([ [ 'printer_id', "=", $printer_id ] ])->findOrEmpty()->toArray();
        if (empty($info)) {
            throw new CommonException('PRINTER_NOT_EXIST');
        }

        $yly_config = new YlyConfig($info[ 'open_id' ], $info[ 'apikey' ]);
        $access_token = $this->getYlyToken($yly_config, true);
        return $access_token;
    }
    ...
    public function testYlyPrint($printer)
    {
        ...
        try {
            ...
            $access_token = $this->getYlyToken($config);
