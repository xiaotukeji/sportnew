...
class ExportService extends BaseAdminService
{
    ...
    public function getPage(array $where = [])
    {
        ...
        $search_model = $this->model->where([['id', '>', 0]])->withSearch(['export_key', 'export_status', 'create_time'],$where)->append(['export_key_name', 'export_status_name'])->field($field)->order('id desc');
    ...
    public function exportData(string $type, array $where){
        ...
        ExportJob::dispatch(['type' => $type, 'where' => $where, 'page' => $page]);
        return true;
    }
    ...
    {
        $export = $this->model->where([['id', '=', $id]])->find();
