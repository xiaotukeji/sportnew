<template>
    <view class="create-event-page">
                ...
                <view class="settings-card">
                    ...
                    <view class="card-content">
                        <view class="co-organizer-section">
                            <view class="co-organizer-list" v-if="coOrganizers && coOrganizers.length > 0">
                                <view 
                                    v-for="(organizer, index) in coOrganizers" 
                                    :key="organizer.id || index"
                                    class="co-organizer-item"
                                >
                                    <view class="organizer-info">
                                        <view class="organizer-logo" v-if="organizer.logo">
                                            <image :src="organizer.logo" class="logo-image" />
                                        </view>
                                        <view class="organizer-details">
                                            <text class="organizer-name">{{ organizer.organizer_name }}</text>
                                            <text class="organizer-type">{{ getOrganizerTypeLabel(organizer.organizer_type) }}</text>
                                            <text class="organizer-contact" v-if="organizer.contact_name">{{ organizer.contact_name }} - {{ organizer.contact_phone }}</text>
                                        </view>
                                    </view>
                                    <view class="organizer-actions">
                                        <button class="edit-btn" @tap="editCoOrganizer(organizer, index)">编辑</button>
                                        <button class="delete-btn" @tap="deleteCoOrganizer(index)">删除</button>
                                    </view>
                                </view>
                            </view>
                            ...
                            <button class="add-co-organizer-btn" @tap="showCoOrganizerModal">
                                <text class="btn-text">+ 添加协办单位</text>
                            </button>
                        </view>
                    </view>
                </view>
        ...
        <!-- 协办单位弹窗 -->
        <view v-if="showCoOrganizerModal" class="popup-mask" @tap="closeCoOrganizerModal">
            <view class="popup-container co-organizer-popup" @tap.stop>
                <view class="popup-header">
                    <text class="popup-title">{{ isEditingCoOrganizer ? '编辑协办单位' : '添加协办单位' }}</text>
                    <text class="popup-close" @tap="closeCoOrganizerModal">×</text>
                </view>
                <view class="popup-content">
                    <view class="form-group">
                        <text class="form-label">单位类型：</text>
                        <radio-group @change="onOrganizerTypeChange" class="radio-group">
                            <label class="radio-item">
                                <radio value="1" :checked="coOrganizerForm.organizer_type === 1" />
                                <text class="radio-text">协办单位</text>
                            </label>
                            <label class="radio-item">
                                <radio value="2" :checked="coOrganizerForm.organizer_type === 2" />
                                <text class="radio-text">赞助商</text>
                            </label>
                            <label class="radio-item">
                                <radio value="3" :checked="coOrganizerForm.organizer_type === 3" />
                                <text class="radio-text">支持单位</text>
                            </label>
                        </radio-group>
                    </view>
                    ...
                    <view class="form-group">
                        <text class="form-label">单位Logo：</text>
                        <view class="logo-upload">
                            <view v-if="coOrganizerForm.logo" class="logo-preview">
                                <image :src="coOrganizerForm.logo" class="preview-image" />
                                <button class="remove-logo-btn" @tap="removeCoOrganizerLogo">删除</button>
                            </view>
                            <button v-else class="upload-logo-btn" @tap="chooseCoOrganizerLogo">
                    ...
                    <view class="form-group">
                        ...
                        <textarea 
                            class="form-textarea" 
                            v-model="coOrganizerForm.remark"
                            placeholder="请输入备注信息（可选）"
                        />
                    </view>
                </view>
                <view class="popup-actions">
                    <button class="popup-btn cancel-btn" @tap="closeCoOrganizerModal">取消</button>
                    <button class="popup-btn confirm-btn" @tap="saveCoOrganizer">保存</button>
                </view>
            </view>
        </view>
...
const isEditingCoOrganizer = ref(false)
...
// 协办单位相关方法
const getOrganizerTypeLabel = (type: number) => {
...
const showCoOrganizerModal = () => {
    resetCoOrganizerForm()
    isEditingCoOrganizer.value = false
    editingCoOrganizerIndex.value = -1
    showCoOrganizerModal.value = true
}
...
const loadCoOrganizers = async () => {
    ...
    try {
        const { getCoOrganizerList } = await import('@/addon/sport/api/co-organizer')
        const response: any = await getCoOrganizerList(eventForm.value.id)
        coOrganizers.value = response.data || []
    } catch (error: any) {
        console.error('加载协办单位失败:', error)
        coOrganizers.value = []
    }
}

const resetCoOrganizerForm = () => {
    coOrganizerForm.value = {
        organizer_type: 1,
        organizer_name: '',
        logo: '',
...
const editCoOrganizer = (organizer: any, index: number) => {
    coOrganizerForm.value = { ...organizer }
    isEditingCoOrganizer.value = true
    editingCoOrganizerIndex.value = index
    showCoOrganizerModal.value = true
}

const deleteCoOrganizer = async (index: number) => {
    const organizer = coOrganizers.value[index]
    
    uni.showModal({
        title: '确认删除',
        content: '确定要删除这个协办单位吗？',
        success: async (res) => {
            if (res.confirm) {
                try {
                    if (organizer.id) {
                        const { deleteCoOrganizer } = await import('@/addon/sport/api/co-organizer')
                        await deleteCoOrganizer(organizer.id)
                    }
                    
                    coOrganizers.value.splice(index, 1)
                    uni.showToast({
                        title: '删除成功',
                        icon: 'success'
                    })
...
const chooseCoOrganizerLogo = () => {
    uni.chooseImage({
        ...
        success: (res) => {
            ...
            coOrganizerForm.value.logo = tempFilePath
        }
    })
}

const removeCoOrganizerLogo = () => {
    coOrganizerForm.value.logo = ''
}
...
const saveCoOrganizer = async () => {
    ...
    try {
        if (isEditingCoOrganizer.value) {
            // 编辑模式
            const { updateCoOrganizer } = await import('@/addon/sport/api/co-organizer')
            await updateCoOrganizer(coOrganizerForm.value.id!, coOrganizerForm.value)
            
            // 更新本地数据
            coOrganizers.value[editingCoOrganizerIndex.value] = { ...coOrganizerForm.value }
            ...
            // 新增模式
            const { addCoOrganizer } = await import('@/addon/sport/api/co-organizer')
            const response: any = await addCoOrganizer({
                ...coOrganizerForm.value,
                event_id: eventForm.value.id || 0
            })
            ...
            coOrganizers.value.push(newOrganizer)
