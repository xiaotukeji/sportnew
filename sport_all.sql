...
-- ----------------------------
-- Table structure for sport_event_groups
-- 赛事自定义分组表
-- ----------------------------
DROP TABLE IF EXISTS `sport_event_groups`;
CREATE TABLE `sport_event_groups` (
  `id` int(11) unsigned NOT NULL AUTO_INCREMENT COMMENT '分组ID',
  `event_id` int(11) unsigned NOT NULL COMMENT '赛事ID',
  `group_name` varchar(255) NOT NULL DEFAULT '' COMMENT '分组名称',
  `group_type` varchar(50) NOT NULL DEFAULT 'custom' COMMENT '分组类型：custom自定义',
  ...
  KEY `idx_event_id` (`event_id`),
  KEY `idx_status_sort` (`status`,`sort`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='赛事自定义分组表';
