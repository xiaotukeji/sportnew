...
import { getSportBaseItemInfo,addSportBaseItem,editSportBaseItem } from '@/addon/sport/api/sport_base_item';
...
const setFormData = async (id:number = 0) => {
    ...
    const data = await (await getSportBaseItemInfo(id)).data
...
const onSave = async (formEl: FormInstance | undefined) => {
    ...
    await formEl.validate(async (valid) => {
       if (valid) {
           ...
           const save = id ? editSportBaseItem : addSportBaseItem
           save(data).then(res => {
               loading.value = false
               history.back()
           }).catch(err => {
